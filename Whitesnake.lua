-- [[ 1. CONFIGURATION ]]
local WEBHOOK_URL = "-- [[ 1. CONFIGURATION ]]
...
local MSG_FILE = "WebhookMsgID_" .. player.UserId .. ".txt"
...
-- [[ 3. WEBHOOK SYSTEM (แก้ไขข้อความเดิม) ]]
...
-- [[ 4. AUTO FARM & ANTI-BUG FUNCTIONS ]]
local function collectItems()
    local items = workspace:GetDescendants()
    if not items then return end -- กันบัค table nil
    
    for _, item in ipairs(items) do -- ใช้ ipairs อย่างปลอดภัย
...
-- [[ 5. MAIN LOOP ]]
print("🎬 เริ่มระบบ Fully Auto (Edit Mode)...")
while task.wait(3) do
    ...
    if amount > 0 then
        ...
        if success and result then -- เช็คข้อมูลเซิร์ฟเวอร์ก่อนวนลูปเพื่อกันบัค
